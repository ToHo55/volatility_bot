...
class Backtesting:
    ...
    def load_price_data(self, ticker):
        ...
        try:
            if ticker not in self.price_cache:
                ...
                # 시작일부터 종료일까지의 데이터 로드 (최대 3번 재시도)
                for attempt in range(3):
                    try:
                        # 백테스팅 기간에 맞는 데이터 로드
                        df = pyupbit.get_ohlcv(ticker, 
                                             interval="minute5",
                                             count=1000)  # 최대 5일치 데이터
                        
                        if df is not None and not df.empty:
                            # 백테스팅 기간에 해당하는 데이터만 필터링
                            df = df[self.start_date:self.end_date]
                            if not df.empty:
                                self.price_cache[ticker] = df
                                logger.info(f"{ticker} 가격 데이터 로드 완료 ({len(df)} 개)")
                                return df
                            else:
                                logger.warning(f"{ticker} 해당 기간의 데이터가 없습니다.")
                        else:
                            logger.warning(f"{ticker} 가격 데이터 로드 실패 (시도 {attempt + 1}/3)")
                        
                        time.sleep(1)  # 1초 대기 후 재시도
                    except Exception as e:
                        logger.error(f"{ticker} 가격 데이터 로드 중 오류 (시도 {attempt + 1}/3): {str(e)}")
                        if "Too many API requests" in str(e):
                            time.sleep(2)  # API 제한 시 2초 대기
                        else:
                            time.sleep(1)  # 기타 오류 시 1초 대기
                
                logger.error(f"{ticker} 가격 데이터 로드 최종 실패")
                return None
            
            return self.price_cache[ticker]
            
        except Exception as e:
            logger.error(f"{ticker} 가격 데이터 로드 중 예외 발생: {str(e)}")
            return None

    def get_price_at_time(self, ticker, current_time):
        """특정 시점의 가격 데이터 조회"""
        try:
            ...
            logger.error(f"{ticker} 가격 조회 중 오류: {str(e)}")
            return None
    ...
    def check_holdings(self, current_time):
        ...
        for ticker, data in list(self.holdings.items()):
            try:
                # 현재가 조회 (백테스팅 시점 기준)
                price_data = self.get_price_at_time(ticker, current_time)
    ...
    def find_buying_opportunities(self, current_time, tickers):
        ...
        for ticker in tickers:
            ...
            try:
                ...
                # 거래량 필터링
                if filtered_df['volume'].mean() < Settings.Market.MIN_TRADE_VOLUME:
                    continue

                # 매수 신호 확인
                analysis = self.strategy_composer.analyze(ticker, filtered_df, current_price)
                if analysis and analysis['total_confidence'] >= Settings.Trading.CONFIDENCE_THRESHOLD:
                    logger.info(f"{ticker} 매수 신호 감지 (신뢰도: {analysis['total_confidence']:.2f}%)")
                    self.execute_buy(ticker, current_price, current_time, analysis['total_confidence'])
    ...
    def print_daily_summary(self, date):
        ...
        try:
            ...
            # 일별 수익률 계산
            daily_return = ((self.balance - self.initial_balance) / self.initial_balance) * 100
            
            logger.info(f"\n===== {date.date()} 거래 요약 =====")
            logger.info(f"총 거래: 매수 {buys}건, 매도 {sells}건")
            logger.info(f"수익 거래: {len(profits)}건")
            logger.info(f"손실 거래: {len(losses)}건")
            logger.info(f"현재 잔고: {self.balance:,.0f}원")
            logger.info(f"누적 수익률: {daily_return:.2f}%")
            logger.info(f"보유 코인 수: {len(self.holdings)}개")
            logger.info("========================\n")
            
        except Exception as e:
            logger.error(f"일별 요약 출력 중 오류 발생: {str(e)}")
    ...
    def run_backtest(self, tickers=None):
        ...
        # 백테스팅 시작 전에 모든 코인의 데이터를 미리 로드
        logger.info("백테스팅 데이터 로드 중...")
        loaded_count = 0
        failed_count = 0
        
        for ticker in tickers:
            if self.load_price_data(ticker) is not None:
                loaded_count += 1
            else:
                failed_count += 1
            ...
            if total % 10 == 0:  # 10개마다 진행상황 출력
                logger.info(f"데이터 로드 진행 중... ({total}/{len(tickers)}) - 성공: {loaded_count}, 실패: {failed_count}")
        
        logger.info(f"백테스팅 데이터 로드 완료 - 총 {len(tickers)}개 중 {loaded_count}개 성공, {failed_count}개 실패")
        
        if loaded_count == 0:
            logger.error("데이터를 로드하지 못했습니다. 백테스팅을 중단합니다.")
            return
            
        # 실제 백테스팅은 데이터가 있는 코인만 대상으로 진행
        valid_tickers = [ticker for ticker in tickers if ticker in self.price_cache]
        logger.info(f"백테스팅 대상 코인: {len(valid_tickers)}개")

        current_date = self.start_date
        while current_date <= self.end_date:
            self.simulate_day(current_date, valid_tickers)
    ...
    def simulate_day(self, date, tickers):
        ...
        try:
            # 초기 자본금 대비 손실률 계산
            current_loss_rate = ((self.balance - self.initial_balance) / self.initial_balance) * 100
            
            # 손실률이 -10%를 초과하면 백테스팅 종료
            if current_loss_rate <= -10:
                logger.warning(f"손실률 {current_loss_rate:.2f}%가 -10%를 초과하여 백테스팅을 종료합니다.")
                self.print_backtest_results()
                self.plot_results()
                self.save_results()
                sys.exit(0)
            ...
            current_profit_rate = ((self.balance - self.initial_balance) / self.initial_balance) * 100
            if current_profit_rate >= 30:
                logger.info(f"목표 수익률 {current_profit_rate:.2f}%를 달성하여 백테스팅을 종료합니다.")
                self.print_backtest_results()
                self.plot_results()
                self.save_results()
                sys.exit(0)
            ...
            for minute in range(0, 24*60, 5):
                ...
                # 매 시간마다 상태 출력
                if minute % 60 == 0:
                    logger.info(f"시간: {current_time.strftime('%H:%M')} - 현재 잔고: {self.balance:,.0f}원, 보유 코인: {len(self.holdings)}개")

            # 하루 종료 시 거래 요약 출력
            self.print_daily_summary(date)
            logger.info(f"=== {date.date()} 거래 종료 ===\n")
