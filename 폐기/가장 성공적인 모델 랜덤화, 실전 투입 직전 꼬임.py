...
class RSIReversalStrategy:
    ...
    def calculate_dynamic_targets(self, df, entry_price, signal_strength=1.0, market_condition=None):
        # ATR(14) 또는 표준편차 등 변동성 지표 활용
        atr = df['close'].rolling(14).std().iloc[-1]
        # 신호 강도(거래량, 캔들크기 등) 반영
        base_tp = 2.5 * atr * signal_strength
        base_sl = 1.2 * atr / max(signal_strength, 0.1)
        base_trail = 1.0 * atr
        # 개선1: 익절폭 확대
        base_tp *= 1.5
        # 개선2: 손절폭 축소
        ...
        base_trail *= 1.5
        # 개선4: 시장상태별 차등 적용
        if market_condition in [MarketCondition.STRONG_UPTREND, MarketCondition.STRONG_UPTREND_HIGH_VOL]:
            base_tp *= 1.3
            base_sl *= 1.2
            base_trail *= 1.2
        elif market_condition in [MarketCondition.STRONG_DOWNTREND, MarketCondition.STRONG_DOWNTREND_HIGH_VOL]:
            base_tp *= 0.8
            base_sl *= 0.8
            base_trail *= 0.8
        # 최대 보유시간: 변동성 클수록 길게, 작을수록 짧게 (최소 1시간, 최대 6시간)
        base_hold = int(24 + min(60, atr * 10000))  # 24~84틱(2~7시간)
        return entry_price + base_tp, entry_price - base_sl, base_trail, base_hold, base_tp

    def evaluate_entry(self, df: pd.DataFrame, progress_queue=None) -> Tuple[bool, str, float]:
        try:
            ...
            recent_candles = df['close'].diff().iloc[-3:]
            num_down = (recent_candles < 0).sum()
            num_up = (recent_candles > 0).sum()
            recent_candle = (df['close'].iloc[-1] - df['open'].iloc[-1]) / df['open'].iloc[-1]
            if not hasattr(self, 'signal_count'):
                self.signal_count = 0
            signal_strength = 1.0
            # 진입 조건 대폭 완화 및 DEBUG 로그 추가
            if market_condition in [MarketCondition.STRONG_UPTREND, MarketCondition.STRONG_UPTREND_HIGH_VOL]:
                prev_high = df['high'].rolling(20).max().iloc[-2]
                cond1 = (current_price > prev_high and volume > avg_volume * 0.8 and current_rsi > 45)
                cond2 = (current_price > bb_upper.iloc[-1]*0.97 and current_rsi > 50)
                if cond1 or cond2:
                    # print(f"[DEBUG] 상승장 진입조건 충족: {self.ticker} {df.index[-1]} rsi={current_rsi:.2f} price={current_price:.2f} bb_upper={bb_upper.iloc[-1]:.2f} vol={volume:.2f} avg_vol={avg_volume:.2f}")
                    msg = f"{self.ticker} 상승장 진입 신호 발생: {df.index[-1]}"
                    if progress_queue is not None:
                        progress_queue.put((self.ticker, 'signal', msg))
                    self.signal_count += 1
                    signal_strength = max(1.0, (volume / avg_volume) * (1 + abs(recent_candle)))
                    idx = df.index.get_loc(df.index[-1])
                    for offset in [5, 10, 20]:
                        if idx + offset < len(df):
                            future_price = df['close'].iloc[idx + offset]
                            ret = (future_price - current_price) / current_price
                            with open('entry_curve_data.csv', 'a', encoding='utf-8') as ef:
                                ef.write(f'{market_condition.value},{offset},{ret}\n')
                    return True, "LONG", signal_strength
            elif market_condition in [MarketCondition.SIDEWAYS, MarketCondition.SIDEWAYS_HIGH_VOL]:
                cond1 = (current_rsi < 45 and current_price < bb_lower.iloc[-1]*1.10 and volume > avg_volume * 0.8)
                cond2 = (current_price < bb_lower.iloc[-1]*1.05)
                if cond1 or cond2:
                    # print(f"[DEBUG] 횡보장 진입조건 충족: {self.ticker} {df.index[-1]} rsi={current_rsi:.2f} price={current_price:.2f} bb_lower={bb_lower.iloc[-1]:.2f} vol={volume:.2f} avg_vol={avg_volume:.2f}")
                    msg = f"{self.ticker} 횡보장 진입 신호 발생: {df.index[-1]}"
                    if progress_queue is not None:
                        progress_queue.put((self.ticker, 'signal', msg))
                    self.signal_count += 1
                    signal_strength = max(1.0, (volume / avg_volume) * (1 + abs(recent_candle)))
    ...
    def evaluate_exit(self, df: pd.DataFrame, position: str, entry_price: float, hold_time: float, signal_strength: float = 1.0, market_condition=None, partial_exit_state=None) -> tuple:
        try:
            current_price = df['close'].iloc[-1]
            profit_pct = ((current_price - entry_price) / entry_price * 100)
            take_profit, stop_loss, trail, max_hold, tp_gap = self.calculate_dynamic_targets(df, entry_price, signal_strength, market_condition)
            # 트레일링스탑: 진입 후 최고가/최저가 추적
            ...
            # 개선5: 부분 익절(익절 목표의 70% 도달 시 50% 청산)
            if partial_exit_state is None and current_price >= entry_price + tp_gap * 0.7:
                return True, "부분익절", 0.5  # 50% 청산
            # 익절/손절/트레일링스탑/최대보유
            if current_price >= take_profit:
                return True, "동적익절", 1.0
            if current_price <= stop_loss:
                return True, "동적손절", 1.0
            if self.max_profit_since_entry >= trail and profit_pct < (self.max_profit_since_entry - trail):
                return True, "동적트레일링스탑", 1.0
            if hold_time >= max_hold / 12:  # 1틱=5분, 12틱=1시간
                return True, "동적최대보유", 1.0
            return False, "", 0.0
        except Exception as e:
            logger.error(f"청산 조건 평가 중 오류: {str(e)}")
            return True, "오류", 1.0    

    def process_single_window(self, window: pd.DataFrame) -> list:
        try:
            ...
            if self.loop_counter % 1000 == 0:
                print(f"[LOOP] {self.ticker} {current_time} 루프 {self.loop_counter}회 진행 중...")
            ...
            if self.current_position is None:
                should_enter, entry_type, signal_strength = self.evaluate_entry(window, progress_queue=None)
                if should_enter:
                    stop_loss, _, _, _, _ = self.calculate_dynamic_targets(window, current_price, signal_strength, market_condition)
                    ...
                    if self.position_size > 0:
                        print(f"[ENTRY] {self.ticker} {window.index[-1]} 진입! position_size={self.position_size}")
                        ...
                        print(f"[DEBUG] ENTRY 기록: trades={len(self.trades)} total_trades={self.summary.total_trades} successful_trades={self.summary.successful_trades}")
            else:
                hold_time = (current_time - self.entry_time).total_seconds() / 3600
                signal_strength = getattr(self, 'signal_strength', 1.0)
                partial_exit_state = getattr(self, 'partial_exit_state', None)
                should_exit, exit_reason, exit_ratio = self.evaluate_exit(window, self.current_position, self.entry_price, hold_time, signal_strength, market_condition, partial_exit_state)
                if should_exit:
                    if exit_reason == "부분익절" and (partial_exit_state is None):
                        # 부분 익절: 포지션의 50%만 청산, 나머지는 trailing stop 적용
                        print(f"[PARTIAL EXIT] {self.ticker} {window.index[-1]} 부분익절! position_size={self.position_size*0.5}")
                        self.log_trade_details('EXIT', current_price,
                            position=self.current_position,
                            profit_pct=((current_price - self.entry_price) / self.entry_price * 100) * 0.5,
                            reason=exit_reason,
                            market_condition=market_condition.value)
                        self.position_size *= 0.5
                        self.partial_exit_state = 'partial'
                    else:
                        if self.current_position == 'long':
                            exit_price = current_price * (1 - FEE_RATE - SLIPPAGE)
                            profit_pct = ((exit_price - self.entry_price) / self.entry_price * 100) * self.position_size
                        else:
                            exit_price = current_price * (1 + FEE_RATE + SLIPPAGE)
                            profit_pct = ((self.entry_price - exit_price) / self.entry_price * 100) * self.position_size
                        print(f"[EXIT] {self.ticker} {window.index[-1]} 청산! profit_pct={profit_pct:.2f}% reason={exit_reason}")
                        self.log_trade_details('EXIT', current_price,
                            position=self.current_position,
                            profit_pct=profit_pct,
                            reason=exit_reason,
                            market_condition=market_condition.value)
                        print(f"[DEBUG] EXIT 기록: trades={len(self.trades)} total_trades={self.summary.total_trades} successful_trades={self.summary.successful_trades}")
                        self.current_position = None
                        self.entry_price = 0
                        self.entry_time = None
                        self.position_size = 0
    ...
    def calculate_position_size(self, current_price: float, stop_loss_pct: float) -> float:
        """
        변동성 및 신호 강도 기반 동적 포지션 크기 산출 (예시: 최대 손실 2% 제한)
        """
        try:
            max_risk = DEFAULT_MAX_RISK_PER_TRADE  # 2%
            # stop_loss_pct가 0에 가까우면 최소값 보정
            stop_loss_pct = max(stop_loss_pct, 0.002)
            # 전체 자산의 몇 %를 진입할지 계산 (예: 최대 손실 2% 이내)
            position_size = max_risk / stop_loss_pct
            # 0~1 사이로 제한, 최소 0.01 강제
            position_size = min(max(position_size, 0.01), 0.2)
    ...
    try:
        ...
        for i in range(total_len):
            ...
            if len(window) >= strategy.bb_period:
                # evaluate_entry에 progress_queue 전달
                if strategy.current_position is None:
                    should_enter, entry_type, signal_strength = strategy.evaluate_entry(window, progress_queue=progress_queue)
                    if should_enter:
                        stop_loss, _, _ = strategy.calculate_dynamic_exits(window, float(window['close'].iloc[-1]))
                        stop_loss_pct = (stop_loss / float(window['close'].iloc[-1]) - 1) if entry_type == "LONG" else (1 - stop_loss / float(window['close'].iloc[-1]))
                        strategy.position_size = strategy.calculate_position_size(float(window['close'].iloc[-1]), abs(stop_loss_pct))
                        # print(f"[DEBUG] 진입 시도: {self.ticker} {window.index[-1]} position_size={self.position_size}")
                        if strategy.position_size > 0:
                            strategy.current_position = entry_type.lower()
                            strategy.entry_price = float(window['close'].iloc[-1]) * (1 + FEE_RATE + SLIPPAGE) if entry_type == "LONG" else float(window['close'].iloc[-1]) * (1 - FEE_RATE - SLIPPAGE)
                            strategy.entry_time = window.index[-1]
                            strategy.trailing_stop_price = 0
                            strategy.signal_strength = signal_strength  # 진입 신호 강도 저장
                            strategy.log_trade_details('ENTRY', float(window['close'].iloc[-1]),
                                position=strategy.current_position,
                                position_size=strategy.position_size,
                                reason="진입 조건 충족",
                                market_condition=market_condition.value)
                            print(f"[DEBUG] run_strategy ENTRY: trades={len(strategy.trades)} total_trades={strategy.summary.total_trades} successful_trades={strategy.summary.successful_trades}")
                else:
                    hold_time = (window.index[-1] - strategy.entry_time).total_seconds() / 3600
                    signal_strength = getattr(strategy, 'signal_strength', 1.0)  # 진입 시 저장된 신호강도 사용
                    should_exit, exit_reason = strategy.evaluate_exit(window, strategy.current_position, 
                                                                    strategy.entry_price, hold_time, signal_strength)
                    if should_exit:
                        if strategy.current_position == 'long':
                            exit_price = float(window['close'].iloc[-1]) * (1 - FEE_RATE - SLIPPAGE)
                            profit_pct = ((exit_price - strategy.entry_price) / strategy.entry_price * 100)
                        else:  # short
                            exit_price = float(window['close'].iloc[-1]) * (1 + FEE_RATE + SLIPPAGE)
                            profit_pct = ((strategy.entry_price - exit_price) / strategy.entry_price * 100)
                        strategy.log_trade_details('EXIT', float(window['close'].iloc[-1]),
                            position=strategy.current_position,
                            profit_pct=profit_pct,
                            reason=exit_reason,
                            market_condition=market_condition.value)
                        print(f"[DEBUG] run_strategy EXIT: trades={len(strategy.trades)} total_trades={strategy.summary.total_trades} successful_trades={strategy.summary.successful_trades}")
                        strategy.current_position = None
                        strategy.entry_price = 0
                        strategy.entry_time = None
                        strategy.position_size = 0
                        strategy.trailing_stop_price = 0
                        strategy.signal_strength = 1.0  # 포지션 종료 후 초기화
            if (i+1) % 100 == 0 or i+1 == total_len:
...
def parameter_search(ticker, start_date, end_date):
    # 탐색할 파라미터 후보군
    rsi_period_list = [6, 8, 10, 12]
    bb_period_list = [12, 15, 18]
    bb_std_list = [1.5, 1.7, 2.0]
    best_result = None
    best_params = None
    ...
    for rsi_period in rsi_period_list:
        for bb_period in bb_period_list:
            for bb_std in bb_std_list:
                strategy = RSIReversalStrategy(ticker, is_backtest=True, randomize=False)
                ...
                total_profit = strategy.summary.total_profit_pct
                print(f"{ticker} | rsi:{rsi_period}, bb_period:{bb_period}, bb_std:{bb_std} -> 수익률: {total_profit:.2f}%")
                if (best_result is None) or (total_profit > best_result):
                    best_result = total_profit
                    best_params = (rsi_period, bb_period, bb_std)
    print(f"\n[{ticker}] 최적 파라미터: rsi:{best_params[0]}, bb_period:{best_params[1]}, bb_std:{best_params[2]} | 수익률: {best_result:.2f}%")
    ...
    try:
        ...
        for repeat_idx in range(NUM_REPEAT):
            ...
            for idx, ticker in enumerate(tickers):
                # 각 티커별 데이터 길이 미리 계산
                df_tmp = fetch_ohlcv(ticker, start_date_str, end_date_str)
                if df_tmp is not None:
                    total_len = len(df_tmp)
                else:
                    total_len = 1
                bars[ticker] = tqdm(total=total_len, desc=ticker, position=idx, leave=True, bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}] {postfix}')
                bar_data[ticker] = {'total': total_len}
        ...
        # 4개 코인, 120일 구간 자동탐색
        from datetime import datetime, timedelta
        tickers = ["KRW-XRP", "KRW-BCH", "KRW-ADA", "KRW-DOGE"]
        ...
        for ticker in tickers:
            print(f"\n===== {ticker} 파라미터 자동탐색 시작 =====")
            parameter_search(ticker, start_date_str, end_date_str)
    except Exception as e:
