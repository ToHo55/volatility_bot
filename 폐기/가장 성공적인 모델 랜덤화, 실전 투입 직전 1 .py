...
def print_data_progress(current: float, total: float, month: str, prefix: str = '', length: int = 40):
    """데이터 수집 진행률을 프로그레스 바로 표시"""
    percent = min(100, (current / total * 100))
    filled_length = int(length * current // total)
    bar = '█' * filled_length + '-' * (length - filled_length)
    print(f'\r{LogColors.INFO}{prefix} |{bar}| {percent:.1f}% - {month} 수집 중{LogColors.RESET}', end='')
    if percent >= 100:
        print()
...
def fetch_ohlcv(ticker: str, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        for month_start, cache_file in missing_months:
            ...
            # 진행률 초기화
            print_data_progress(0, 1, month_str, prefix=f"{ticker}")
            ...
            while current_to >= month_start:
                for retry in range(5):
                    try:
                        ...
                        # 진행률 업데이트
                        progress = (month_end - current_to) / (month_end - month_start)
                        print_data_progress(progress, 1, month_str, prefix=f"{ticker}")
...
class RSIReversalStrategy:
    ...
    def _confirm_entry(self, df: pd.DataFrame, entry_type: str) -> bool:
        try:
            ...
            # 변동성 체크 (메이저 코인은 더 낮은 기준 적용)
            volatility = df['close'].pct_change().rolling(6).std().iloc[-1]  # 12 -> 6으로 단축
            if volatility < self.volatility_threshold:
                return False
            
            # 거래량 체크 (메이저 코인은 더 낮은 기준)
            volume_ratio = df['volume'].iloc[-1] / df['volume'].rolling(6).mean().iloc[-1]
            min_volume_ratio = 0.3 if 'BTC' in self.ticker or 'ETH' in self.ticker else 0.5
            
            if entry_type == "LONG":
                # 메이저 코인은 중간밴드 기준으로 변경
                if 'BTC' in self.ticker or 'ETH' in self.ticker:
                    price_condition = current_price < bb_middle.iloc[-1] * 1.001  # 약간의 버퍼
                else:
                    price_condition = current_price < bb_lower.iloc[-1] * 1.002
                ...
                if 'BTC' in self.ticker or 'ETH' in self.ticker:
                    price_condition = current_price > bb_middle.iloc[-1] * 0.999
                else:
                    price_condition = current_price > bb_upper.iloc[-1] * 0.998
    ...
    def evaluate_entry(self, df: pd.DataFrame) -> Tuple[bool, str]:
        try:
            ...
            # 가격 변화율 계산 (더 짧은 기간)
            price_change = df['close'].pct_change(2)  # 3 -> 2로 단축
            current_change = price_change.iloc[-1]
            
            # RSI 기반 진입 조건 (매우 완화)
            if current_rsi < 55:  # 50 -> 55로 완화
                entry_type = "LONG"
                if current_change < 0:  # -0.001 -> 0으로 완화
                    if self._confirm_entry(df, "LONG"):
                        return True, entry_type
            
            elif current_rsi > 45:  # 50 -> 45로 완화
                entry_type = "SHORT"
                if current_change > 0:  # 0.001 -> 0으로 완화
...
if __name__ == "__main__":
    try:
        ...
        for ticker in tickers:
            try:
                logger.info(f"\n{LogEmoji.MARKET} {ticker} 분석 시작")
                
                # 데이터 가져오기
                logger.info(f"{LogEmoji.DATA} {ticker} 데이터 로드 중...")
                df = fetch_ohlcv(ticker, start_date_str, end_date_str)
                if df is None or len(df) < MIN_DATA_POINTS:
                    logger.warning(f"{LogEmoji.WARNING} {ticker}: 충분한 데이터가 없습니다")
                    continue
                
                logger.info(f"{LogEmoji.SUCCESS} {ticker} 데이터 로드 완료 (총 {len(df)} 개)")
                ...
                for i in range(len(df)):
                    ...
                    strategy.update_progress(i+1, len(df))
                ...
                logger.error(f"{LogEmoji.ERROR} {ticker} 처리 중 오류: {str(e)}")
                continue
