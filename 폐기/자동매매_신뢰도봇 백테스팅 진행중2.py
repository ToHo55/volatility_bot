...
class Settings:
    class Trading:
        ...
        MAX_HOLDINGS = 2                # 보유 가능한 최대 코인 수 (3 -> 2로 감소)
        CHECK_INTERVAL = 5              # 사이클 간격 (초)

        # 매매 전략 설정
        CONFIDENCE_THRESHOLD = 75       # 종합 신뢰도 기준값 상향 (65 -> 75)
        TAKE_PROFIT_PERCENT = 2.0      # 익절 목표
        STOP_LOSS_PERCENT = 2.0        # 손절 기준 완화 (1.5 -> 2.0)
        FORCE_SELL_MINUTES = 180       # 강제 청산 시간 연장 (120 -> 180)

        # 트레일링 스탑 설정
        TRAILING_START = 1.0           # 수익률이 1.0% 이상일 때 트레일링 스탑 시작
        TRAILING_DISTANCE = 0.5        # 최고점 대비 0.5% 하락 시 매도

    class Technical:
        # RSI 설정
        RSI_PERIOD = 14
        RSI_OVERSOLD = 35              # 과매도 기준값 상향 (30 -> 35)
        RSI_OVERBOUGHT = 65            # 과매수 기준값 하향 (70 -> 65)
        RSI_WEIGHT = 0.30              # RSI 가중치 조정 (0.35 -> 0.30)

        # 볼린저 밴드 설정
        BB_WINDOW = 20
        BB_STD_DEV = 2.0              # 표준편차 범위 축소 (2.2 -> 2.0)
        BB_WEIGHT = 0.30              # BB 가중치 조정 (0.35 -> 0.30)

        # MACD 설정 추가
        MACD_WEIGHT = 0.25            # MACD 가중치 상향
        MACD_FAST = 12
        MACD_SLOW = 26
        MACD_SIGNAL = 9
        ...
        MA_WEIGHT = 0.15              # 이동평균선 가중치

        # 변동성 돌파 설정
        VOLATILITY_K = 0.5            # 변동성 계수 상향 (0.4 -> 0.5)
        VOLATILITY_WEIGHT = 0.15      # 변동성 가중치 조정 (0.2 -> 0.15)

    class Market:
        # 거래량 필터링 강화
        MIN_TRADE_VOLUME = 150000     # 최소 거래량 상향 (100000 -> 150000)
...
class MACDStrategy(TradingStrategy):
    ...
    def analyze(self, ticker, df, current_price):
        try:
            if len(df) < Settings.Technical.MACD_SLOW + 10:  # 충분한 데이터 확인
                return None

            # MACD 계산
            exp1 = df['close'].ewm(span=Settings.Technical.MACD_FAST, adjust=False).mean()
            exp2 = df['close'].ewm(span=Settings.Technical.MACD_SLOW, adjust=False).mean()
            macd = exp1 - exp2
            signal = macd.ewm(span=Settings.Technical.MACD_SIGNAL, adjust=False).mean()
            hist = macd - signal

            # 추가 지표 계산
            hist_momentum = hist.diff()  # 히스토그램 모멘텀
            macd_slope = macd.diff()    # MACD 기울기
            volume_trend = df['volume'].rolling(window=5).mean().diff()  # 거래량 추세

            current_macd = macd.iloc[-1]
            current_signal = signal.iloc[-1]
            current_hist = hist.iloc[-1]
            current_momentum = hist_momentum.iloc[-1]
            current_macd_slope = macd_slope.iloc[-1]
            ...
            return {
                ...
                'macd_slope': current_macd_slope,
                'volume_trend': current_volume_trend,
                'strategy': 'MACD'
            }

        except Exception as e:
            logging.error(f"MACD 분석 중 오류: {str(e)}")
            return None
    ...
    def calculate_confidence(self, analysis_result):
        try:
            ...
            # MACD 신호 강도
            macd_strength = abs(analysis_result['macd'])
            if macd_strength > 0:
                confidence += min(20, macd_strength * 10)

            # 히스토그램 모멘텀
            if analysis_result['momentum'] > 0:
                confidence += 10
            else:
                confidence -= 10

            # MACD 기울기
            if analysis_result['macd_slope'] > 0:
                confidence += 15
            else:
                confidence -= 15

            # 거래량 추세 반영
            if analysis_result['volume_trend'] > 0:
                confidence += 5

            # 매수 신호 강화
            if analysis_result['macd'] > analysis_result['signal'] and analysis_result['histogram'] > 0:
                confidence += 20
            elif analysis_result['macd'] < analysis_result['signal'] and analysis_result['histogram'] < 0:
                confidence -= 20
    ...
    def check_holdings(self, current_time):
        ...
        for ticker, data in list(self.holdings.items()):
            try:
                ...
                # 트레일링 스탑 로직 추가
                if profit_rate >= Settings.Trading.TRAILING_START:
                    if 'max_price' not in data:
                        data['max_price'] = current_price
                    elif current_price > data['max_price']:
                        data['max_price'] = current_price
                    
                    trailing_stop_price = data['max_price'] * (1 - Settings.Trading.TRAILING_DISTANCE / 100)
                    if current_price <= trailing_stop_price:
                        self.execute_sell(ticker, current_price, current_time, 'trailing_stop')
                        continue
