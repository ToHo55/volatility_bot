...
class RSIBBStrategy(TradingStrategy):
    ...
    def calculate_confidence(self, analysis_result):
        ...
        if bb_position <= 0.2:  # 하단 밴드 근처
            ...
            bb_confidence = max(0, 100 - (bb_position * 100))
        ...
        ) / (Settings.Technical.RSI_WEIGHT + Settings.Technical.BB_WEIGHT + Settings.Technical.MA_WEIGHT)
...
class Backtesting:
    def __init__(self, start_date, end_date, initial_balance=10000000):
        ...
        logging.info("캐시 디렉토리 확인: " + self.cache_dir)

    def get_cache_filename(self, ticker):
        """캐시 파일 이름 생성"""
        return os.path.join(self.cache_dir, f"{ticker}_price_data.csv")

    def load_cached_price_data(self, ticker):
        """캐시된 가격 데이터 로드"""
        ...
        if os.path.exists(cache_file):
            try:
                ...
                logging.info(f"{ticker} 캐시된 데이터 로드 성공")
                return df
            except Exception as e:
                logging.error(f"{ticker} 캐시 파일 로드 중 오류 발생: {str(e)}")
        return None

    def save_price_data_to_cache(self, ticker, df):
        """가격 데이터를 캐시 파일로 저장"""
        try:
            cache_file = self.get_cache_filename(ticker)
            df.to_csv(cache_file)
            logging.info(f"{ticker} 가격 데이터 캐시 저장 완료")
        except Exception as e:
            logging.error(f"{ticker} 캐시 파일 저장 중 오류 발생: {str(e)}")

    def is_cache_valid(self, df):
        """캐시된 데이터가 유효한지 확인"""
        if df is None or df.empty:
            return False
        ...
        # 백테스팅 기간을 포함하는지 확인
        return data_start <= self.start_date and data_end >= self.end_date

    def load_price_data(self, ticker):
        """가격 데이터 로드 (캐시 활용)"""
        # 메모리 캐시 확인
        if ticker in self.price_cache:
            if self.is_cache_valid(self.price_cache[ticker]):
                logging.info(f"{ticker} 메모리 캐시 사용")
                return self.price_cache[ticker]
        
        # 파일 캐시 확인
        cached_data = self.load_cached_price_data(ticker)
        ...
        # 새로운 데이터 로드
        try:
            logging.info(f"{ticker} 새로운 데이터 로드 중...")
            df = pyupbit.get_ohlcv(ticker, interval="minute5", count=1000)
            if df is not None and not df.empty:
                self.price_cache[ticker] = df
                self.save_price_data_to_cache(ticker, df)
                return df
        except Exception as e:
            logging.error(f"{ticker} 가격 데이터 로드 중 오류: {str(e)}")
    ...
    def print_daily_summary(self, date):
        ...
        try:
            ...
            daily_trades = [trade for trade in self.trade_history 
                          if trade['time'].date() == date.date()]
            
            # 일별 거래 통계 계산
            buys = len([t for t in daily_trades if t['action'] == 'buy'])
            sells = len([t for t in daily_trades if t['action'] == 'sell'])
            
            # 수익/손실 계산
            profits = [t['profit_loss'] for t in daily_trades 
                      if t['action'] == 'sell' and t['profit_loss'] > 0]
            losses = [t['profit_loss'] for t in daily_trades 
                     if t['action'] == 'sell' and t['profit_loss'] <= 0]
            
            # 일별 수익률 계산
            daily_return = ((self.current_balance - self.initial_balance) / self.initial_balance) * 100
            
            logging.info(f"\n===== {date.date()} 거래 요약 =====")
            logging.info(f"총 거래: 매수 {buys}건, 매도 {sells}건")
            logging.info(f"수익 거래: {len(profits)}건")
            logging.info(f"손실 거래: {len(losses)}건")
            logging.info(f"현재 잔고: {self.current_balance:,.0f}원")
            logging.info(f"누적 수익률: {daily_return:.2f}%")
            logging.info(f"보유 코인 수: {len(self.holdings)}개")
            logging.info("========================\n")
            
        except Exception as e:
            logging.error(f"일별 요약 출력 중 오류 발생: {str(e)}")
        ...
        for ticker, data in list(self.holdings.items()):
            try:
                ...
                if profit_rate >= Settings.Trading.TAKE_PROFIT_PERCENT:
                    ...
                    logging.info(f"{ticker} 익절 조건 충족 (수익률: {profit_rate:.2f}%)")
                # 손절
                elif profit_rate <= -Settings.Trading.STOP_LOSS_PERCENT:
                    should_sell = True
                    sell_reason = "stop_loss"
                    logging.info(f"{ticker} 손절 조건 충족 (손실률: {profit_rate:.2f}%)")
                # 강제 청산
                elif (current_time - data['buy_time']).total_seconds() / 60 > Settings.Trading.FORCE_SELL_MINUTES:
                    should_sell = True
                    sell_reason = "force_sell"
                    logging.info(f"{ticker} 강제 청산 (보유시간 초과)")
    ...
    def print_backtest_results(self):
        ...
        logging.info(f"최종 자본금: {final_balance:,.0f}원")
        ...
        if total_trades > 0:
            ...
            # 수익/손실 거래 상세 분석
            profit_trades = [t for t in self.trade_history 
                           if t['action'] == 'sell' and t['profit_loss'] > 0]
            loss_trades = [t for t in self.trade_history 
                          if t['action'] == 'sell' and t['profit_loss'] <= 0]
            
            avg_profit = sum(t['profit_loss'] for t in profit_trades) / len(profit_trades) if profit_trades else 0
            avg_loss = sum(t['profit_loss'] for t in loss_trades) / len(loss_trades) if loss_trades else 0
            
            logging.info(f"평균 수익: {avg_profit:.2f}%")
            logging.info(f"평균 손실: {avg_loss:.2f}%")
            
            # 매도 이유별 통계
            reasons = {'take_profit': 0, 'stop_loss': 0, 'force_sell': 0}
            for trade in self.trade_history:
                if trade['action'] == 'sell':
                    reasons[trade['reason']] = reasons.get(trade['reason'], 0) + 1
            
            logging.info("\n[매도 이유별 통계]")
            logging.info(f"익절: {reasons['take_profit']}회")
            logging.info(f"손절: {reasons['stop_loss']}회")
            logging.info(f"강제 청산: {reasons['force_sell']}회")
    ...
    def plot_results(self):
        ...
        try:
            ...
            times = [trade['time'] for trade in self.trade_history]
            
            plt.figure(figsize=(12, 6))
            plt.plot(times, balances, label='Portfolio Value')
            ...
            logging.error(f"결과 시각화 중 오류 발생: {str(e)}")
    ...
    def save_results(self):
        ...
        try:
            ...
            logging.info("백테스팅 결과가 'backtest_results.json'에 저장되었습니다.")
    ...
    def find_buying_opportunities(self, current_time, tickers):
        ...
        # 현재 보유 중인 코인이 최대 보유 개수에 도달하면 매수 기회를 찾지 않음
        ...
        for ticker in tickers:
            try:
                ...
                # 현재 시점 이전의 데이터만 필터링
                mask = df.index <= current_time
                filtered_df = df[mask].tail(100)  # 최근 100개 데이터만 사용
                
                if filtered_df.empty or len(filtered_df) < 20:  # 최소 20개의 데이터 포인트 필요
                    continue
                ...
                analysis_result = strategy.analyze(ticker, filtered_df, current_price)
                confidence = analysis_result['total_confidence']

                # 신뢰도가 임계값을 넘고, 현재 보유 코인 수가 최대값보다 작을 때만 매수 기회로 판단
                if confidence >= Settings.Trading.CONFIDENCE_THRESHOLD:
                    opportunities.append({
                        'ticker': ticker,
                        'price': current_price,
                        'confidence': confidence
                    })
                    logging.info(f"{ticker} 매수 신호 감지 (신뢰도: {confidence:.2f}%)")

            except Exception as e:
                logging.error(f"{ticker} 매수 기회 탐색 중 오류: {str(e)}")
                continue

        # 남은 보유 가능 개수만큼만 반환 (신뢰도 높은 순으로 정렬)
        opportunities.sort(key=lambda x: x['confidence'], reverse=True)
        return opportunities[:Settings.Trading.MAX_HOLDINGS - current_holdings]
    ...
    def check_volume_requirement(self, ticker, current_time):
        ...
        try:
            ...
            # 현재 시점 이전의 데이터만 필터링
            mask = df.index <= current_time
            filtered_df = df[mask].tail(12)  # 최근 1시간(5분 * 12) 데이터 사용
            ...
            return avg_volume >= Settings.Market.MIN_TRADE_VOLUME
    ...
    def record_daily_balance(self, date):
        ...
        try:
            ...
            logging.info(f"{date.date()} 마감 - 총자산: {total_value:,.0f}원 (수익률: {daily_return:.2f}%)")
